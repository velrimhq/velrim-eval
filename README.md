...
**The bake-off:** six extraction setups (ours included) over 124 real documents, scored by this CLI under a plan frozen before the first paid call. Write-up: [BAKE-OFF.md](./BAKE-OFF.md) · plan: [ANALYSIS-PLAN.md](./ANALYSIS-PLAN.md) · every conflict and limitation: [DISCLOSURES.md](./DISCLOSURES.md) · the held-out split, provable: [SALT.md](./SALT.md). Re-scoring the published outputs needs no keys and costs nothing.

Run data ships in the repo: `results/matrix-out/<arm>/<class>/{main,probe}/` holds every raw per-field output (`predictions.repeat-NNN.jsonl`, with request IDs), the per-repeat `score.repeat-NNN/scores.json`, and the run manifests, health and cost logs; `results/archive/` keeps the first invocation's manifest and cost log plus a copy of the final manifest (the second invocation's manifest was overwritten before it was archived); `results/manifests/` the frozen split manifests. The source documents are `corpora/pdfs/<class>/`. `*.prior-<epoch>` / `*.stale-<epoch>` files are archived event trails and checkpoints from earlier invocations into the same output dir; a live `run-events.jsonl` is empty when the run finished with no circuit/resume events. Before publication, local paths in the run metadata were neutralized to `<local-pdfs>/…` and `<run-manifests>/…` and the Vertex project id to `<gcp-project>`; the split-manifest `path` entries inside each `run-manifest.json` stay as recorded — they are inputs to the run fingerprints. Nothing else was edited.
...
- `run-manifest.json` records the commit/worktree state, individual executed-file hashes, exact schema/public-golden hashes, frozen source-golden hashes, shared instruction, actual installed scoring version, requested pins, allowlisted returned versions/request IDs, fixture corpus hash (fixture mode), and stable run-window timestamps. `publicationReady` stays false with an explicit `missingFields` list until every later protocol gate is present. For the published matrix, that gate is matrix-level: `validation.publicationReady: true` in `results/matrix-out/matrix-manifest.json`; the per-run flags stay `false` by design, listing `matrix.authoritativeValidation` as the one missing field.
- What actually goes on the wire: **velrim** POSTs `{ schema, document: { bytes_base64 } }` to `/v1/extract`; **openai** attaches the PDF as a base64 `file` content part with the schema in the prompt (Chat Completions, `logprobs: true`; `--structured-mode` adds `response_format: json_schema` — constrained decoding); **gemini** POSTs the same schema-in-prompt text plus the PDF as inline base64 to `generateContent` — AI Studio by default (key in the `x-goog-api-key` header), or Vertex with `--gemini-vertex-project <id>`, which is how the published bake-off ran (`--structured-mode` adds `generationConfig.responseJsonSchema`); **llamaextract** uploads the file, creates a stateless extract job with your schema inline (`data_schema`), and polls it to completion; **mistral** POSTs the PDF as a base64 data URL to `/v1/ocr` with the schema as `document_annotation_format` — the returned `document_annotation` JSON is what maps to fields.
